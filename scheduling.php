...
if (isset($_POST['pass_role'])) {
    ...
    if ($userRankingResult->num_rows > 0) {
        ...
        $nextUserQuery = "SELECT member_id FROM faculty_members WHERE ranking = $userRanking + 1 LIMIT 1";        
...
$recordsQuery = "
    SELECT t.timetable_id, fm.member_name, s.subject_name, sec.section_name, se.session_name, c.classroom_name, lv.level_name, d.department_name
    FROM timetable t
    INNER JOIN member_courses mc ON t.member_course_id = mc.member_course_id
    ...
    <table border='1' cellpadding='10'>
        ...
        if ($recordsResult) {
            while ($row = $recordsResult->fetch_assoc()) {
                ...
                echo "<a href='delete_scheduling.php?id=" . $row['timetable_id'] . "' onclick='return confirm(\"هل أنت متأكد أنك تريد حذف هذا السجل؟\");'>حذف</a>";
                echo "</td>";
                echo "</tr>";
            }
        } else {
